...
public class CreateActionDialog extends Dialog<Action> {
    ...
    public CreateActionDialog(String taskOutputPath, Action action) {
        ...
        this.imageChooser.getExtensionFilters().add(
        ...
        actionComboBox.setOnAction(e -> {
            updateDynamicFields(actionComboBox.getValue());
    ...
    private void updateActionComboBox(String fileType) {
        ...
        if (!fileType.equals("All Files")) {
            acp.acp_project.Models.File selectedFile = null;
            switch (fileType) {
                case "jpeg":
    ...
    private void addWatermarkImageField(int row) {
        Label label = new Label("Watermark Image:");
        TextField imagePathField = new TextField();
        imagePathField.setEditable(false);
        Button button = new Button("Select Image");
        button.setOnAction(e -> {
            File file = imageChooser.showOpenDialog(getDialogPane().getScene().getWindow());
            if (file != null) {
                imagePathField.setText(file.getAbsolutePath());
            }
        });
        HBox hbox = new HBox(10, imagePathField, button);
        grid.add(label, 0, row);
        grid.add(hbox, 1, row);
        dynamicFields.put("WatermarkImage", imagePathField);
    }
    ...
    private void populateDynamicFields(Action action) {
        for (Map.Entry<String, String> entry : action.getActionParameters().entrySet()) {
            ...
            if (control instanceof TextField) {
                ...
                ((ComboBox<String>) control).setValue(entry.getValue());
            }
        }
    }
}
